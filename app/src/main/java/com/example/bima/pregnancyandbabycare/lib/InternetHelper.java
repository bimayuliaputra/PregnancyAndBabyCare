...
public class InternetHelper {
    ...
    public static final String URI="http://192.168.8.106/monitoring/v1/api";
    ...
    public static String sendHttpRequest(String method, String urlString, ArrayList<KeyValue> data) throws Exception{
        ...
        if(connection.getResponseCode() == -1){
            ...
            throw new Exception("Cannot connect to host" + connection.getURL().toString());
        }
    ...
    public static String uploadFiles(String urlString, FormData data) throws Exception{
        ...
        for(KeyValue fileData : allFileData){
            ...
            outputStream.writeBytes(twoHypens + boundary + crlf);
            outputStream.writeBytes("Content-Disposition: form-data; name=\"" + attachmentName +
        ...
        for(KeyValue plainDta : allPlainData){
            outputStream.writeBytes(twoHypens + boundary + crlf);
            outputStream.writeBytes("Content-Disposition: form-data; name=\"" + plainDta.getKey() + "\"" + crlf);
            outputStream.writeBytes("Content-Type: text/plain" + crlf);
            outputStream.writeBytes(crlf);
            outputStream.writeBytes(plainDta.getValue().toString());
            outputStream.writeBytes(crlf);
        }
        outputStream.flush();
        outputStream.close();
        ...
        if(responseCode != HttpURLConnection.HTTP_OK){
            throw new Exception("Failed (Response Code : " + responseCode + connection.getURL().toString());
        }
        String response = InternetHelper.retrieveResponseString(connection);
        return response;
    }
